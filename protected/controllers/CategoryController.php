...
class CategoryController extends Controller
{
    ...
    public function actionView($id)
    {
        ...
        if ($a_category) {
            $a_review = Review::model()->with(array('product.a_simple.simple.category' => array(
                'condition' => 'parent_id=' . $o_category->primaryKey),
            ))->findAllByAttributes(
                array('status' => 1),
                array('order' => 't.id DESC', 'limit' => Review::ON_PAGE_CATEGORY)
            );
            $count = Review::model()->with(array('product.a_simple.simple.category' => array(
                'condition' => 'parent_id=' . $o_category->primaryKey),
            ...
            $a_product = Product::model()->with(array('a_simple.simple' => array(
    ...
    public function actionMore($id)
    {
        ...
        if ($a_category) {
            ...
            $a_review = Review::model()->with(array('product.a_simple.simple' => array(
    ...
    public function actionCheck($id)
    {
        ...
        if ($a_category) {
            ...
            $count = Review::model()->with(array('product.a_simple.simple' => array(
                'condition' => 'category_id=' . $id),
            ))->countByAttributes(
                array('status' => 1)
            );
